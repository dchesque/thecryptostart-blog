...
export function buildOpenApiSpec() {
    ...
    const adminPaths: Array<{ method: 'get' | 'post' | 'put' | 'delete' | 'patch'; path: string; summary: string; body?: any; params?: any; query?: any; tag: string; resp200?: any; resp201?: any }> = [
        ...
        { method: 'post', path: '/api/admin/cleanup', summary: 'Retention sweeps (SystemLog/SpamLog/PendingSubscribers/PasswordReset). Designed for an external daily cron.', tag: 'Admin: Ops', body: z.object({ systemLogDays: z.number().int().min(1).max(365).optional(), spamLogDays: z.number().int().min(7).max(730).optional(), pendingSubscriberDays: z.number().int().min(1).max(365).optional(), passwordResetExpired: z.boolean().optional(), dryRun: z.boolean().optional() }), resp200: z.object({ success: z.boolean(), dryRun: z.boolean(), totalDeleted: z.number().int(), ranAt: z.string(), result: z.any() }) },
